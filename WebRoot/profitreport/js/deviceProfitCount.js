...
Ext.onReady(function() {
	...
	var deviceProfitStore = new Ext.data.JsonStore({
	...
	selectdeviceProfitForm = new Ext.form.FormPanel(
			{
				title : '查询条件',
				renderTo : 'deviceProfit',
				labelAlign : 'right',
				buttonAlign : 'center',
				layout : 'column',
				region : 'north',
				...
				buttons : [{
								...
								handler : function() {
									...
									deviceProfitStore.proxy = new Ext.data.HttpProxy(
											{
								...
								iconCls : 'icon-reset',
								handler : function() {
									selectdeviceProfitForm.form.reset();
								},
								cls : 'right-part'// ,
							}]
			});
	
	var deviceProfitGrid = new Ext.grid.GridPanel({
		renderTo : 'deviceProfit',
		store : deviceProfitStore,
		id : 'deviceProfitGrid',
		...
		tbar : [{
			...
			iconCls : "icon-print",
			handler : function() {
				var vExportContent = deviceProfitGrid.getExcelXml(); // 获取数据
		...
		new Ext.Viewport({
			layout : 'border',
			items : [selectdeviceProfitForm,deviceProfitGrid]
		});
