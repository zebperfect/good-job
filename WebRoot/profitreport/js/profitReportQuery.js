...
Ext.onReady(function() {
	...
	var profitReportQueryStore = new Ext.data.JsonStore({
	...
	selectprofitReportQueryForm = new Ext.form.FormPanel(
			{
				title : '查询条件',
				renderTo : 'profitReportQuery',
				labelAlign : 'right',
				buttonAlign : 'center',
				layout : 'column',
				region : 'north',
				...
				buttons : [{
								...
								handler : function() {
									...
									profitReportQueryStore.proxy = new Ext.data.HttpProxy(
											{
									...
									profitReportQueryStore.load( {
												params : {
													start : 0,
													limit : 20,
													'flowReport.username' : Ext.getCmp('flowReport.username.show').getValue(),
								...
								iconCls : 'icon-reset',
								handler : function() {
									selectprofitReportQueryForm.form.reset();
								},
								cls : 'right-part'// ,
							}]
			});
	
	var profitReportQueryGrid = new Ext.grid.GridPanel({
		renderTo : 'profitReportQuery',
		store : profitReportQueryStore,
		sm : sm,
		id : 'profitReportQueryGrid',
		...
		tbar : [{
			...
			iconCls : "icon-print",
			handler : function() {
				var vExportContent = profitReportQueryGrid.getExcelXml(); // 获取数据
			...
			text : '<font color="red">*注：系统只支持查询近三月（含当月）的分润，默认加载本月1号到当前时间的分润和。</font>'
		}],
		bbar : [{
					xtype : 'paging',
					pageSize : 20,
		...
		new Ext.Viewport({
			layout : 'border',
			items : [selectprofitReportQueryForm,profitReportQueryGrid]
		});
